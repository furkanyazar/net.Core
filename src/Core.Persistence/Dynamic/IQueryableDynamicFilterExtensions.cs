...
public static class IQueryableDynamicFilterExtensions
{
    ...
    public static string Transform(Filter filter, IList<Filter> filters)
    {
        ...
        if (
            ...
            || !Operators.TryGetValue(filter.Operator, out string? comparison)
        )
        ...
        if (!string.IsNullOrEmpty(filter.Value))
            ...
            else if (comparison is "StartsWith" or "EndsWith" or "Contains")
            {
                ...
                {
                    where.Append(
                        $"(np({filter.Field}).ToLower().{comparison}(@{index.ToString()}.ToLower()))"
                    );
                }
                else
                {
            ...
            else if (filter.Operator == "in")
            {
                ...
                var paramIndexes = Enumerable
                    .Range(index, valueCount)
                    .Select(i => $"@{i}")
                    .ToArray();
                ...
                {
                    where.Append(
                        $"np({filter.Field}).ToLower() in ({string.Join(",", paramIndexes)})"
                    );
                }
                else
                {
            ...
            else if (filter.Operator == "between")
            {
                ...
                where.Append(
                    $"(np({filter.Field}) >= {lowerBound} and np({filter.Field}) <= {upperBound})"
                );
            }
            else
                where.Append($"np({filter.Field}) {comparison} @{index.ToString()}");
